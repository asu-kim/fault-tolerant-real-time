...
- Baseline (`EvalNoAdvance.lf`): No LET advance, only retrying until deadline misses.
- Proposed1 (`EvalWorstCase.lf`): Advance LET as much as `wcet_f` even if the segment succeeds, and do proactive task abortion.
- Proposed2 (`EvalApproach.lf`): Advance LET as much as `wcet_f`when segment fails, and `wcet_s` when segment succeeds, and do proactive task abortion.
...
In the proposed approaches, there are LF's deadline handlers, but it the deadline is never violated due to proactive task dropping.
